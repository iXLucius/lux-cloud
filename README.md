# AG-Admin
AG-Admin是国内首个基于`Spring Cloud`微`服务`化`开发平台`，具有统一授权、认证后台管理系统，其中包含具备用户管理、资源权限管理、网关API管理等多个模块，支持多业务系统并行开发，可以作为后端服务的开发脚手架。代码简洁，架构清晰，适合学习和直接项目中使用。核心技术采用Consul、Fegin、Ribbon、Zuul、Hystrix、`JWT Token`、Mybatis、Zipkin、boot-admin等主要框架和中间件，前端采用`vue-element-admin`组件。 
...
### 运行步骤
- 运行数据库脚本：依次运行数据库：lux-admin/db/init.sql、lux-auth-server/db/init.sql
- 修改配置数据库配置：lux-admin/src/main/resources/application.yml、lux-gate/src/main/resources/application.yml
- 按`顺序`运行main类：ZipkinStreamServerApplication（lux-tracker）、AdminBootstrap（lux-admin）、AuthBootstrap（lux-auth-server）、GateBootstrap（lux-gate）
